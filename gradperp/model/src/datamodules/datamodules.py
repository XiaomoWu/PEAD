...
class FrDataModule(BaseDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.train_val_dataset = FrDataset(
        ...
        n_train_val_split = [
            round(x * len(self.train_val_dataset)) for x in self.train_val_split
        ]
    ...
    def collate_fn(self, data):
        # Unpack a batch
        docid_idx, t, mantxts, finratios, auxcars, auxvols, fund, revision, retail = (
            zip(*data)
        )
...
class FrTxtDataModule(BaseDataModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert datamodule_txt_return_type in ["padded_tensor", "packed_tensor"]
    ...
    def collate_fn(self, data):
        ...
        for section_type in ["md", "qa"]:
            ...
            # trim to max_doc_len
            doc_lens = [min(d[section_type].size(0), self.max_doc_len) for d in docs]
            ...
            doc_embs_trimmed = [
                torch.nn.functional.normalize(
                    d[section_type][:max_doc_len, :], p=2, dim=1
                )
                # d[section_type][:max_doc_len, :]
                for d in docs
            ]

            # creawte type tokens, which are the same shape as the embs
            doc_typetokens_trimmed = [
                torch.tensor([section_token] * d.size(0), dtype=torch.int64)
                for d in doc_embs_trimmed
            ]
            ...
            if self.datamodule_txt_return_type == "padded_tensor":
                # debug only
                # for i, emb in enumerate(doc_embs_trimmed):
                #     print(docid_idx[i], emb.size())

                # pad
                doc_embs_trimmed_padded = torch.nn.utils.rnn.pad_sequence(
                    doc_embs_trimmed, batch_first=True, padding_value=0.0
                )  # (N, S, E)

                doc_typetokens_trimmed_padded = torch.nn.utils.rnn.pad_sequence(
                    doc_typetokens_trimmed, batch_first=True, padding_value=0.0
                )

                # maks: True (masked), False (not masked)
                attention_mask = torch.tensor(
                    [[False] * l + [True] * (max_doc_len - l) for l in doc_lens],
                    dtype=torch.bool,
                )
    ...
    def _trim_to_max_doc_len(self, doc_tokens=None, doc_embs=None):
        ...
        selector = [
            [True] * min(self.max_doc_len, n) + [False] * max(0, n - self.max_doc_len)
            for n in counts
        ]
