...
class BaseDataset(Dataset):
    ...
    def __init__(
        self, tasks, tx_df_name, split_df_name, split_type, split_id, data_dir
    ):
        """
        Args:
        ...
        _, train_start_date, train_end_date, test_start_date, test_end_date, _ = tuple(
            split_df.loc[(split_df.split_id == split_id)].iloc[0]
        )
        ...
        if split_type == "train_val":
            tx_df = tx_df[tx_df.rdq.between(train_start_date, train_end_date)]

        elif split_type == "test":
            tx_df = tx_df[tx_df.rdq.between(test_start_date, test_end_date)]
    ...
    def _get_auxcars(self, sample):
        """
        This function extract cars (e.g, CAR5). They intend to be used as INPUTS.

        Under normal circumstances, we shouldn't use `_get_auxcars`. We only need it when we
        want to use auxiliary CARs as inputs to predict the primary CARs, i.e., peak into future.
        """
        if "tx_v1" in self.tx_df_name:
            ...
            return (
                car_dgtw_call_0_3_std,
                car_dgtw_call_0_21_std,
                car_dgtw_call_0_63_std,
            )
        elif "tx_v3" in self.tx_df_name:
            return ()
        else:
            raise ValueError("tx_df_name is not valid")

    def _get_auxvols(self, sample):
        """
        This function extract volatility (e.g, vol5). They intend to be used as INPUTS.

        Under normal circumstances, we shouldn't use `_get_auxvols`. We only need then when we
        want to use auxiliary volatility as inputs to predict the primary CARs, i.e., peak into future.
        """
        if "tx_v1" in self.tx_df_name:
    ...
    def _get_financial_ratios(self, sample):
        ...
        if "tx_v1" in self.tx_df_name:
            ...
            # if using factor benchmark, include alpha
            if "car_ff5_call" in self.tasks[0]:
                car_m1_m1 = sample.car_ff5_call_m1_m1_std
                car_m2_m2 = sample.car_ff5_call_m2_m2_std
                car_m21_m3 = sample.car_ff5_call_m21_m3_std
                alpha = sample.alpha_ff5_call_std
...
class FrDataset(BaseDataset):
    ...
    def __init__(
        self, split_type, split_id, tx_df_name, split_df_name, tasks, data_dir, **kwargs
    ):
...
class FrTxtDataset(BaseDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "tx_v3" in tx_df_name:
            self.dt_sents = (
                ds.dataset(
                    f"{data_dir}/transcript/dt-sents",
                    format="feather",
                )
                .to_table(columns=["transcriptid", "sentenceid", "section_type"])
                .to_pandas()
            )
    ...
    def _get_doc(self, sample):
        ...
        if "tx_v1" in self.tx_df_name:
            ...
            preemb = torch.load(f"{self.preemb_dir}/{tid}.pt", weights_only=True)

        # group preemb by each section_type
        if 'mpnet' in self.preemb_dir:
            doc = {"md": torch.zeros(1, 768), "qa": torch.zeros(1, 768)}
        elif 'llama' in self.preemb_dir:
            doc = {"md": torch.zeros(1, 4096), "qa": torch.zeros(1, 4096)}

        for section_type in [["md"], ["q", "a"]]:
            sids = dt_sents_sample[
                dt_sents_sample.section_type.isin(section_type)
            ].sentenceid
            ...
            if len(docemb) == 0:
                if 'mpnet' in self.preemb_dir:
                    docemb = torch.zeros(1, 768)
                elif 'llama' in self.preemb_dir:
                    docemb = torch.zeros(1, 4096)
                logger.warning(f"tid {tid} has no {section_type} sentences")
            else:
