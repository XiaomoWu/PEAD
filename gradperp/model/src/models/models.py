...
class FrModel(torch.nn.Module):
    ...
    def get_shared_params(self):
        """All shared parameters (GradCos, GradPerp, GradNorm)"""
        shared_params = (
...
class FrTxtModel(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.doc_encoder_md = self.init_doc_encoder(
            self.d_model, self.nhead, doc_encoding_pooling_method
        )
    ...
    def init_doc_encoder(self, d_model, nhead, doc_encoding_pooling_method):
        if "transformer" in doc_encoding_pooling_method:
...
class FrTxtMQModel(FrTxtModel):
    def __init__(
        ...
        weighting_method_name,
        datamodule_cfg,
        qsz,  # length of task-specific queries, default 48
        mq_enc_dec_times=1,
        **kwargs,
    ):
        """Args:
        ...
        # in MQT, positional embeddings are learned
        del (
            self.pe_md,
            self.pe_qa,
            self.doc_encoder_md,
        ...
        # reduce llama 2 embedding size
        # d_model of llama 2 is 4096, too large to fit into GPU
        # we reduce it to 768, the same as MPNet
        if "llama" in datamodule_cfg.preemb_dir:
            self.reduce_llama_d_model = nn.Linear(4096, d_model)
        ...
        self.enc_dec_md_list = nn.ModuleList(
            [MQEncDec(n_tasks, d_model, nhead, qsz) for i in range(mq_enc_dec_times)]
        )
        ...
        self.fc_output = nn.ModuleList(
            [
                ...
                # MQResidFinalFc(2 * d_model + self.n_enhanced_wide_features)
        ...
        self.output_final_features = kwargs.get("output_final_features", False)
    ...
    def forward(self, batch):
        ...
        # reduce d_model if it's llama 2
        if "llama" in self.datamodule_cfg.preemb_dir:
            sent_embs_md = self.reduce_llama_d_model(sent_embs_md)
            sent_embs_qa = self.reduce_llama_d_model(sent_embs_qa)
        ...
        x = [
            fc(feat.to(fc.weight.dtype))
            for fc, feat in zip(self.fc_output, final_features)
        ]  # list of (B, 1)
        y = torch.cat(x, dim=1)  # (B, n_tasks)

        output = {"y": y}
        if self.output_final_features:
            output["final_features"] = final_features[0]
    ...
    def get_shared_params(self):
        ...
        We only get the last (few) layer(s) because text model has so many params that using all shared params is impossible
        """
        if self.weighting_method_name in ["GradCos", "GradPerp", "OlAux"]:
            shared_params = [
                param
        ...
        elif self.weighting_method_name in ["GradNorm"]:
            shared_params = [
                param
                for name, param in self.named_parameters()
                if param.requires_grad
                and (
                    # method 1: each layer has its own fc layer
                    # only include qa_list to avoid OOM
                    ("enc_dec_qa_list.0.decoder.ffn.fc2" in name)
                    # or ("enc_dec_md_list.0.decoder.ffn.fc2" in name)
                )
            ]
        else:
            shared_params = None
...
class MQEncDec(nn.Module):
    def __init__(self, n_tasks, d_model, nhead, qsz):
        """
        Args:
        ...
        self.encoder = MQEncoder(d_model, nhead)
        self.decoder = MQDecoder(d_model, nhead)
        # self.act = nn.GELU()
    ...
    def _init_weights(self):
        ...
        # init task queries, features and pos
...
class MQEncoder(nn.Module):
    def __init__(self, d_model, nhead):
        ...
        self.enc_attn = nn.MultiheadAttention(
            d_model, num_heads=nhead, dropout=0.1, batch_first=True
        )
...
class MQResidFinalFc(nn.Module):
    """The final FC layer in MQ encoder/decoder (residual connection is included!)"""
...
class MtlModel(pl.LightningModule):
    ...
    # forward
    def forward(self, batch):
        """Extract final features (for MQT)"""
        docid_idx = batch["docid_idx"]
        t = batch["t"][:, 0]  # (N,)
        final_features = self.model(batch).get("final_features", None)
        return docid_idx, final_features, t
    ...
    def test_step(self, batch, batch_idx):
        ...
        y = model_output["y"]
        final_features = model_output.get("final_features", None)

        # get truth
        t = self.get_truth(batch)

        # collect outputs (new in v2.0)
        output = {"docid_idx": batch["docid_idx"], "y": y, "t": t}
        if hasattr(self.model, 'output_final_features'):
            if self.model.output_final_features:
    ...
    def on_test_epoch_end(self):
        ...
        outputs = self.collect_yt(outputs)

        docid_idx = outputs["docid_idx"]
        y = outputs["y"]
        t = outputs["t"]
        final_features = outputs.get("final_features", None)

        # save yt to DataFrame (and wandb)
        yt = self.save_yt(docid_idx, y, t, final_features)
        ...
        self.log("test/EV", test_ev.to(self.device), sync_dist=True)
        self.log(
            "test/N_obs", torch.tensor(yt.shape[0]).to(self.device), sync_dist=True
        )
    ...
    def add_task_weights_param(self):
        ...
        if self.weighting_method_name == "GradNorm":
            self.task_weights = nn.Parameter(
                torch.ones(self.n_tasks, device=self.device) / self.n_tasks
            )
    ...
    def calc_losses(self, y, t):
        ...
        # ev loss
        losses = [1 - torchmetrics.functional.explained_variance(y[:, i], t[:, i]) for i in range(self.n_tasks)]
    ...
    def collect_yt(self, outputs):
        ...
        y = torch.cat([x["y"] for x in outputs])  # (n_sample, n_tasks)
        t = torch.cat([x["t"] for x in outputs])  # (n_sample, n_tasks)
        if "final_features" in outputs[0]:
            final_features = torch.cat(
                [x["final_features"] for x in outputs]
            )  # (n_sample, n_features)

        # all_gather yt
        docid_idx = self.all_gather(docid_idx).detach()  # (n_gpu, n_sample_per_gpu)
        y = self.all_gather(y).detach()  # (n_gpu, n_sample_per_gpu, n_tasks)
        t = self.all_gather(t).detach()  # (n_gpu, n_sample_per_gpu, n_tasks)
        if "final_features" in outputs[0]:
            final_features = self.all_gather(
                final_features
            ).detach()  # (n_gpu, n_sample_per_gpu, n_features)

        assert docid_idx.dim() == 2
        assert y.dim() == t.dim() == 3
        assert y.size(-1) == t.size(-1) == self.n_tasks
        if "final_features" in outputs[0]:
            assert final_features.size(0) == y.size(0)

        # convert yt dimension
        docid_idx = docid_idx.flatten().tolist()  # 1D
        y = y.reshape(-1, self.n_tasks)  # (n_sample, n_tasks)
        t = t.reshape(-1, self.n_tasks)  # (n_sample, n_tasks)

        if "final_features" in outputs[0]:
            final_features = final_features.reshape(
                -1, final_features.size(-1)
            )  # (N, n_features)

        output = {"docid_idx": docid_idx, "y": y, "t": t}
        if "final_features" in outputs[0]:
            output["final_features"] = final_features
        return output
    ...
    def training_step_logging(self, losses):
        ...
        if hasattr(self.weighting_method, "task_weights"):
            for t, w in zip(
                ...
                self.log(
                    f"train/task_weight_{t}",
                    w.detach() if isinstance(w, torch.Tensor) else w,
                )
    ...
    def save_yt(self, docid_idx, y, t, final_features) -> pd.DataFrame:
        # get split_id
        ...
        # get final_features (if exists)
        if final_features is not None:
            yt["final_features"] = final_features.tolist()
        ...
        yt = pd.DataFrame(yt)
        yt = yt.drop_duplicates(subset=["docid_idx"]).reset_index(drop=True)
        ...
        if sweep_id is None:
            savedir = self.data_dir / f"eval/yt-temp/(run_id)-{run_timespec}-{run_id}"
        else:
            savedir = self.data_dir / f"eval/yt-temp/(sweep_id)-{sweep_id}"
