...
class AdaMt(WeightingMethod):
    ...
    def backward(self, losses, pl_module, **kwargs):
        ...
        task_specific_norms = torch.stack(
            [torch.linalg.norm(self._flattening(g)) for g in task_specific_grads]
        )  # shape (n_tasks,)

        assert (
            len(task_specific_norms) == self.n_tasks
        ), f"Require len(task_specific_norms)==n_tasks, but got {task_specific_norms}"
...
class Dwa(WeightingMethod):
    ...
    def backward(self, losses, pl_module, **kwargs):
        ...
        # update task_weights
        # - in the first two epochs don't update task_weights
        if epoch > 1:
...
class Uncert(WeightingMethod):
    ...
    def backward(self, losses, pl_module, **kwargs):
        ...
        if self.uncert_mode == "no_plus1":
            ...
            tot_loss = torch.sum(
                losses * precision + torch.log(1 + torch.exp(pl_module.logvars))
            )
        ...
        for t, p, v in zip(
            pl_module.hparams.datamodule_cfg.tasks, precision, pl_module.logvars
        ):
...
class GradCos(WeightingMethod):
    ...
    def backward(self, losses, pl_module, shared_params, **kwargs):
        ...
        shared_grads = self.get_shared_grads(losses, shared_params=shared_params)
    ...
    def get_shared_grads(self, losses, shared_params):
        ...
        pri_grad = torch.autograd.grad(pri_loss, shared_params, retain_graph=True)

        # init shard_grads
        shared_grads = tuple(g.clone() for g in pri_grad)

        for loss in aux_losses:
            aux_grad = torch.autograd.grad(loss, shared_params, retain_graph=True)
            cosine = self.get_grad_cos_sim(pri_grad, aux_grad)

            if cosine > 0:
                shared_grads = tuple(g + ga for g, ga in zip(shared_grads, aux_grad))
...
class GradNorm(WeightingMethod):
    ...
    def __init__(self, n_tasks, alpha=1.5, **kwargs):
        ...
        self.task_weights = (
            None  # it's just a copy of the true task_weights from the pl_module
        )
    ...
    def backward(self, losses, pl_module, shared_params, **kwargs):
        ...
        # print(f'task_weights: {pl_module.task_weights.detach()}')
        ...
        for w_i, L_i in zip(pl_module.task_weights, losses):
            dlidW = torch.autograd.grad(L_i, shared_params, retain_graph=True)[0]
        ...
        pl_module.task_weights.grad = torch.autograd.grad(
            grad_norm_loss, pl_module.task_weights, retain_graph=True
        )[0]
...
class OlAux(WeightingMethod):
    ...
    def backward(self, losses, pl_module, shared_params, **kwargs):
        ...
        pri_grads = torch.autograd.grad(
            pri_loss, shared_params, retain_graph=True, allow_unused=True
        )
        # Note:
        ...
        aux_grads = [
            self._flattening(tuple(g.clone() for g in aux_g)) for aux_g in aux_grads
        ]
        ...
        if (pl_module.global_step >= self.N) and (pl_module.global_step % self.N == 0):
            ...
            task_weights_grads = (
                torch.stack(task_weights_grads).sum(dim=0).to(pl_module.device)
            )  # (n_tasks-1,)
        ...
        tot_loss = torch.sum(pri_loss + torch.sum(aux_losses * self.aux_task_weights))
...
class GradPerp(WeightingMethod):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            M_mode: "min", "max", "mean". M_mode can be used with M. For example,
                if M=4 and M_mode="min", then M=4*min(task_weights)
                - "min": M=min(task_weights)
                - "max": M=max(task_weights)
                - "mean": M=mean(task_weights)
            qr_mode: "diag" or "row"
    ...
    def backward(self, losses, pl_module, shared_params, **kwargs):
        ...
        if global_step >= 5:
            ...
            self.task_weights = self._avg_task_weights(task_weights, global_step)
        ...
        if self.M_mode is not None:
            ...
            if self.M_mode == "max":
                # i.e., M \lambda_0 = max(\lambda_1, \lambda_2)
                M = (self.task_weights[1:].max())/self.task_weights[0]
                self.task_weights[0] = self.task_weights[1:].max() * self.M
                pl_module.log('train/M', self.M*M)
            # elif self.M_mode == "min":
            #     self.task_weights[0] *= self.task_weights[1:].min()
            # elif self.M_mode == "mean":
            #     self.task_weights[0] *= self.task_weights[1:].mean()
        else:
            if self.M > 0:
    ...
    def get_G(self, losses, shared_params):
        ...
        G = [
            self._flattening(torch.autograd.grad(l, shared_params, retain_graph=True))
            for l in losses
        ]
    ...
    def _avg_G(self, G, G_sq, global_step, epoch, n_train_batches):
        ...
        if self.avg_G is None:
            ...
            self.avg_G = (1 - self.beta1) * G + self.beta1 * self.avg_G
        ...
        if self.beta2 is not None:
            if self.avg_G_sq is None:
                ...
                self.avg_G_sq = (1 - self.beta2) * G_sq + self.beta2 * self.avg_G_sq

            avg_G_sq_corrected = (
                1 / (1 - self.beta2 ** (global_step + 1))
            ) * self.avg_G_sq
    ...
    def _avg_task_weights(self, task_weights, global_step):
        if self.avg_task_weights is None:
            ...
            self.avg_task_weights = (
                1 - self.beta1
            ) * task_weights + self.beta1 * self.avg_task_weights
    ...
    def _avg_losses(self, losses, global_step):
        ...
        if self.avg_losses is None:
            ...
            self.avg_losses = (1 - self.beta1) * losses + self.beta1 * self.avg_losses
    ...
    def _qr(self, G, pl_module):
        ...
        if self.qr_mode in [
            ...
            assert (task_weights.dim() == 1) and (
                task_weights.shape[0] == self.n_tasks
            ), f"Require task_weights to be a vector, got {task_weights=}"
        ...
        # only for the toy example!
            ...
            for col_idx in range(G.shape[1]):
                cos_score = self._get_grad_cos_sim(G[:, 0], G[:, col_idx])
